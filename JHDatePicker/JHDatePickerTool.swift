...
extension Date{
    ...
    static public func getDateWeekday(date:Date) ->String{
        let weekdaysTitleArr = ["周日","周⼀","周二","周三","周四","周五","周六"]
        let timeInterval:TimeInterval = date.timeIntervalSince1970
        let days = Int(timeInterval/86400)
        let weekday = ((days + 4)%7+7)%7
        return weekdaysTitleArr[weekday]
    }
    static public func jh_setYear(year:NSInteger,month:NSInteger,day:NSInteger,
                              hour:NSInteger,minute:NSInteger,second:NSInteger) -> Date{
